...
public class StickyLiveData<T> {
     ...
     * {@link StickyLiveData#observe(LifecycleOwner, Observer,boolean)} with a LifecycleOwner, which
    ...
    public void observeForever(@NonNull Observer<T> observer,boolean isSticky) {
        ...
        ObserverWrapper existing = mObservers.putIfAbsent(observer, wrapper);
        if (existing != null && existing instanceof StickyLiveData.LifecycleBoundObserver) {
            throw new IllegalArgumentException("Cannot add the same observer"
                    + " with different lifecycles");
        }
        if (existing != null) {
    ...
    private abstract class ObserverWrapper {
        ...
        void activeStateChanged(boolean newActive) {
            ...
            // owner
            mActive = newActive;
            boolean wasInactive = StickyLiveData.this.mActiveCount == 0;
            StickyLiveData.this.mActiveCount += mActive ? 1 : -1;
            if (wasInactive && mActive) {
                onActive();
            }
            if (StickyLiveData.this.mActiveCount == 0 && !mActive) {
                onInactive();
            }
            if (mActive) {
